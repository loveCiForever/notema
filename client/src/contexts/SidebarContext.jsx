...
export const SidebarProvider = ({ children }) => {
  ...
  useEffect(() => {
    const favoriteNotes = sampleNotes.filter((note) => note.favorite && !note.deleted)
    const privateNotes = sampleNotes.filter((note) => !note.public && !note.favorite && !note.deleted)
    const publicNotes = sampleNotes.filter((note) => note.public && !note.deleted)
  ...
  const moveToTrash = (noteId) => {
    setNotes((prev) => prev.map((note) => (note.id === noteId ? { ...note, deleted: true } : note)))
  ...
  const restoreFromTrash = (noteId) => {
    setNotes((prev) => prev.map((note) => (note.id === noteId ? { ...note, deleted: false } : note)))
  ...
  const updateMenuItemsWithNotes = () => {
    const favoriteNotes = notes.filter((note) => note.favorite && !note.deleted)
    const privateNotes = notes.filter((note) => !note.public && !note.favorite && !note.deleted)
    ...
    setMenuItems((prev) =>
      prev.map((item) => {
        if (item.id === sectionId) {
          ...
          if (order === "newest") {
            sortedNotes.sort((a, b) => new Date(b.lastEdited) - new Date(a.lastEdited))
          } else if (order === "oldest") {
            sortedNotes.sort((a, b) => new Date(a.lastEdited) - new Date(b.lastEdited))
